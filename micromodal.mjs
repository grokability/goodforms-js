...
const MicroModal = (() => {
  ...
  class Modal {
    ...
    toggleScrolling (value) {
      ...
      const shouldLock = !value || activeModals.length > 0
      document.body.style.overflow = shouldLock ? 'hidden' : ''
      document.body.style.touchAction = shouldLock ? 'none' : '' // for mobile scrolling
    }
    ...
    getFocusableNodes () {
      const nodes = this.modal.querySelectorAll(FOCUSABLE_ELEMENTS.join(",")) // DAMMIT - not compatible
